...
export default function CreativeHubPage() {
  ...
  const generateMarkdownSummary = () => {
    ...
    data.projects.forEach((p, idx) => {
      ...
      md += `- **ステータス**: ${p.status} (進捗 ${p.progressPercent}%)\n`;
    ...
    data.ideas.forEach((idea) => {
      ...
      md += `### [${idea.category.toUpperCase()}] ${idea.title} (${idea.status})\n`;
  ...
  // --- ヘルパーマッピング ---
  const categoryLabels: Record<ProjectCategory, { label: string; color: string }> = {
    illustration: { label: 'イラスト集・画集', color: 'bg-sky-100 text-sky-800 border-sky-200' },
    comic: { label: '漫画・Webtoon', color: 'bg-amber-100 text-amber-800 border-amber-200' },
    game: { label: 'ゲーム企画', color: 'bg-purple-100 text-purple-800 border-purple-200' },
    novel: { label: '小説・シナリオ', color: 'bg-emerald-100 text-emerald-800 border-emerald-200' },
    goods: { label: 'グッズ・企画', color: 'bg-pink-100 text-pink-800 border-pink-200' },
    other: { label: 'その他創作', color: 'bg-slate-100 text-slate-800 border-slate-200' }
  };

  const statusLabels: Record<ProjectStatus, { label: string; bg: string; text: string }> = {
    concept: { label: '構想中', bg: 'bg-slate-100', text: 'text-slate-700' },
    planning: { label: '企画・プロット中', bg: 'bg-indigo-100', text: 'text-indigo-700' },
    in_progress: { label: '制作進行中', bg: 'bg-blue-100', text: 'text-blue-700' },
    polishing: { label: '仕上げ・校正中', bg: 'bg-amber-100', text: 'text-amber-700' },
    completed: { label: '完成・頒布中', bg: 'bg-emerald-100', text: 'text-emerald-700' },
    archived: { label: 'アーカイブ', bg: 'bg-zinc-200', text: 'text-zinc-600' }
  };
  ...
  const ideaStatusLabels: Record<IdeaStatus, { label: string; badge: string }> = {
    spark: { label: 'ひらめき💡', badge: 'bg-amber-100 text-amber-800 border-amber-300' },
    reviewing: { label: '検討中🤔', badge: 'bg-blue-100 text-blue-800 border-blue-300' },
    accepted: { label: '採用確定✨', badge: 'bg-emerald-100 text-emerald-800 border-emerald-300 font-semibold' },
    shelved: { label: '保留中📦', badge: 'bg-slate-100 text-slate-600 border-slate-300' }
  };

  const kanbanLaneConfig: Record<TaskLane, { label: string; icon: string; desc: string; color: string }> = {
    idea: { label: 'アイデア・構想', icon: '💡', desc: 'ネタ出し・方向性の検討', color: 'border-t-amber-400' },
    plot: { label: 'プロット・構成', icon: '📝', desc: 'ネーム・章立て・設計', color: 'border-t-indigo-400' },
    rough: { label: 'ネーム・下書き', icon: '✏️', desc: 'ラフ画・構図・素案', color: 'border-t-blue-400' },
    production: { label: '本制作・執筆', icon: '🎨', desc: '線画・着色・本文執筆', color: 'border-t-purple-500' },
    review: { label: '仕上げ・校正', icon: '🔍', desc: 'エフェクト・デザイン・校正', color: 'border-t-rose-400' },
    done: { label: '完了・公開', icon: '✨', desc: '頒布準備完了・アーカイブ', color: 'border-t-emerald-500' }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans selection:bg-indigo-500 selection:text-white">
      {/* トースト通知 */}
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-50 bg-indigo-600 text-white px-5 py-3 rounded-xl shadow-2xl flex items-center gap-2 text-sm font-medium border border-indigo-400/30">
      ...
      <header className="border-b border-slate-800 bg-slate-950/80 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link
              href="/"
              className="text-xs font-semibold text-slate-400 hover:text-white flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800/80 hover:bg-slate-700 transition border border-slate-700/50"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              <span>自己紹介ポートフォリオへ</span>
            </Link>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 text-xs font-semibold rounded-lg transition"
            >
              <Lightbulb className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">アイデアをメモ</span>
            </button>
            <button
              onClick={() => {
                setEditingProject({
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-lg shadow-md shadow-indigo-600/30 transition"
            >
      ...
      {/* サマリーバー / 統計情報 */}
      <div className="bg-slate-950/40 border-b border-slate-800/80 py-3">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-2.5 flex items-center justify-between">
              <div>
                <span className="text-slate-400 block text-[11px]">制作プロジェクト</span>
                <span className="text-lg font-bold text-white">{data.projects.length} <span className="text-xs font-normal text-slate-400">件</span></span>
              </div>
              <FolderGit2 className="w-5 h-5 text-indigo-400 opacity-80" />
            </div>

            <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-2.5 flex items-center justify-between">
              <div>
                <span className="text-slate-400 block text-[11px]">ストックアイデア</span>
                <span className="text-lg font-bold text-amber-300">{data.ideas.length} <span className="text-xs font-normal text-slate-400">件</span></span>
              </div>
              <Lightbulb className="w-5 h-5 text-amber-400 opacity-80" />
            </div>

            <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-2.5 flex items-center justify-between">
              <div>
                <span className="text-slate-400 block text-[11px]">進行中タスク</span>
                <span className="text-lg font-bold text-sky-300">{data.tasks.filter(t => t.lane !== 'done').length} <span className="text-xs font-normal text-slate-400">件</span></span>
              </div>
              <KanbanSquare className="w-5 h-5 text-sky-400 opacity-80" />
            </div>

            <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-2.5 flex items-center justify-between">
              <div>
                <span className="text-slate-400 block text-[11px]">世界観用語・プロット</span>
                <span className="text-lg font-bold text-purple-300">{data.lores.length + data.plots.length} <span className="text-xs font-normal text-slate-400">件</span></span>
              </div>
              <BookOpen className="w-5 h-5 text-purple-400 opacity-80" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-4">
          <nav className="flex items-center gap-1.5 bg-slate-800/70 p-1 rounded-xl border border-slate-700/50">
            <button
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <div className="relative flex-1 sm:w-64">
              ...
              <input
                ...
                className="w-full bg-slate-800 border border-slate-700 text-white placeholder-slate-500 text-xs rounded-lg pl-8 pr-3 py-2 focus:outline-none focus:border-indigo-500"
              />
              {searchQuery && (
                <button
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-800/40 p-3 rounded-xl border border-slate-800">
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="text-slate-400 font-medium">絞り込み:</span>
                <select
                  value={selectedCategoryFilter}
                  onChange={(e) => setSelectedCategoryFilter(e.target.value)}
                  className="bg-slate-800 border border-slate-700 text-slate-200 rounded-md px-2 py-1 focus:outline-none"
                >
              ...
              <span className="text-xs text-slate-400">
                表示中: <strong className="text-white">{filteredProjects.length}</strong> 件
              </span>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filteredProjects.map((project) => {
                ...
                return (
                  <div
                    key={project.id}
                    className="bg-slate-800/80 border border-slate-700/80 rounded-2xl p-5 hover:border-indigo-500/50 hover:shadow-xl hover:shadow-indigo-500/5 transition-all flex flex-col justify-between group"
                    style={{ borderTop: `4px solid ${project.color || '#6366f1'}` }}
                  >
                    <div className="space-y-4">
                      {/* カテゴリ & ステータスバッジ */}
                      <div className="flex items-center justify-between text-[11px] gap-2">
                        <span className={`px-2.5 py-0.5 rounded-full font-medium border ${catInfo.color}`}>
                          {catInfo.label}
                        </span>
                        <span className={`px-2 py-0.5 rounded-full font-medium ${statInfo.bg} ${statInfo.text}`}>
                          {statInfo.label}
                        </span>
                      </div>
                      ...
                      {/* 関連データ数カウント */}
                      <div className="flex items-center gap-3 pt-2 text-[11px] text-slate-400 border-t border-slate-700/60">
                        <span className="flex items-center gap-1" title="関連アイデア">
                          <Lightbulb className="w-3.5 h-3.5 text-amber-400" />
                          {projIdeas.length}
                        </span>
                        <span className="flex items-center gap-1" title="タスク">
                          <KanbanSquare className="w-3.5 h-3.5 text-sky-400" />
                          {projTasks.length}
                        </span>
                        <span className="flex items-center gap-1" title="世界観・設定">
                      ...
                      {project.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 pt-1">
                          {project.tags.map((tag, idx) => (
                            <span
                              key={idx}
                              className="text-[10px] px-2 py-0.5 rounded bg-slate-900 text-slate-400 border border-slate-700"
                            >
                              #{tag}
                            </span>
                          ))}
                    ...
                    <div className="pt-4 mt-4 border-t border-slate-700/60 flex items-center justify-between gap-2">
                      <button
                        ...
                        className="flex-1 py-1.5 px-3 bg-indigo-600/20 hover:bg-indigo-600 text-indigo-300 hover:text-white border border-indigo-500/30 text-xs font-semibold rounded-lg transition text-center flex items-center justify-center gap-1"
                      >
                        <span>詳細・設定を開く</span>
            ...
            {filteredProjects.length === 0 && (
              <div className="text-center py-16 bg-slate-800/30 rounded-2xl border border-dashed border-slate-700">
                ...
                <p className="text-slate-400 text-sm">該当するプロジェクトがありません</p>
                <button
                  onClick={() => {
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-800/40 p-3 rounded-xl border border-slate-800">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs rounded-lg transition shadow-md"
              >
                <Plus className="w-3.5 h-3.5" />
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {filteredIdeas.map((idea) => {
                ...
                return (
                  <div
                    key={idea.id}
                    className="bg-slate-800/90 border border-slate-700 rounded-xl p-4 flex flex-col justify-between space-y-3 hover:border-amber-400/50 hover:shadow-lg transition"
                    style={{ borderLeft: `4px solid ${idea.color || '#f59e0b'}` }}
                  >
                    <div className="space-y-2.5">
                      ...
                      <div className="flex items-center justify-between text-[11px] gap-2">
                        ...
                        {proj ? (
                          <span className="px-2 py-0.5 rounded bg-indigo-950 text-indigo-300 border border-indigo-800 text-[10px] font-medium truncate max-w-[120px]">
                            {proj.title}
                          </span>
                        ) : (
                          <span className="text-[10px] text-slate-500">全体ネタ</span>
                        )}
                      </div>
                      ...
                      {/* 内容 */}
                      <p className="text-xs text-slate-300 leading-relaxed whitespace-pre-line bg-slate-900/60 p-2.5 rounded-lg border border-slate-800">
                        {idea.content}
                      </p>

                      {/* 発想メモ */}
                      {idea.inspirationSource && (
                        <p className="text-[11px] text-amber-200/80 italic flex items-start gap-1">
                      ...
                      {idea.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {idea.tags.map((t, idx) => (
                            <span key={idx} className="text-[10px] px-1.5 py-0.5 bg-slate-700/60 text-slate-300 rounded">
                              #{t}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="pt-3 border-t border-slate-700/60 flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        {(['spark', 'reviewing', 'accepted', 'shelved'] as IdeaStatus[]).map((st) => (
                          <button
                            ...
                            className={`px-1.5 py-0.5 text-[10px] rounded transition border ${
                              idea.status === st
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="p-1 text-slate-400 hover:text-white bg-slate-700/40 rounded"
                          title="編集"
                        >
                          <Edit3 className="w-3.5 h-3.5" />
                        </button>
                        <button
                          ...
                          className="p-1 text-rose-400 hover:text-rose-300 bg-rose-500/10 rounded"
                          title="削除"
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
            ...
            {filteredIdeas.length === 0 && (
              <div className="text-center py-16 bg-slate-800/30 rounded-2xl border border-dashed border-slate-700">
                ...
                <p className="text-slate-400 text-sm">アイデアがまだありません</p>
                <button
                  onClick={() => {
                    setEditingIdea({
                      id: '',
        ...
        {activeTab === 'kanban' && (
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-800/40 p-3 rounded-xl border border-slate-800">
              <div className="flex items-center gap-2 text-xs">
                <Filter className="w-3.5 h-3.5 text-slate-400" />
                <span className="text-slate-400">プロジェクト選択:</span>
                <select
                  value={selectedProjectFilter}
                  onChange={(e) => setSelectedProjectFilter(e.target.value)}
                  className="bg-slate-800 border border-slate-700 text-slate-200 rounded-md px-2 py-1 focus:outline-none"
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold text-xs rounded-lg transition shadow-md"
              >
                <Plus className="w-3.5 h-3.5" />
                <span>タスクを追加</span>
              </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 overflow-x-auto pb-4">
              {(['idea', 'plot', 'rough', 'production', 'review', 'done'] as TaskLane[]).map((laneKey) => {
                ...
                return (
                  <div
                    key={laneKey}
                    className={`bg-slate-800/50 border border-slate-700/80 rounded-xl p-3 flex flex-col justify-between min-h-[480px] ${laneConf.color} border-t-4`}
                  >
                    <div>
                      {/* レーンヘッダー */}
                      <div className="flex items-center justify-between pb-2 border-b border-slate-700/60 mb-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <h4 className="text-xs font-bold text-white">{laneConf.label}</h4>
                        </div>
                      ...
                      <div className="space-y-2.5">
                        {laneTasks.map((task) => {
                          ...
                          return (
                            <div
                              key={task.id}
                              className="bg-slate-900 border border-slate-700/80 rounded-lg p-3 space-y-2 shadow-sm hover:border-sky-400/50 transition group"
                            >
                              {/* プロジェクト名 & 優先度 */}
                              ...
                              <div className="pt-2 border-t border-slate-800 flex items-center justify-between text-[11px]">
                                <button
                                  ...
                                  className="p-1 text-slate-400 hover:text-white disabled:opacity-20 disabled:hover:text-slate-400"
                                  title="前工程へ"
                                >
                    ...
                    <button
                      ...
                      className="mt-3 w-full py-1 text-[11px] text-slate-400 hover:text-white hover:bg-slate-700/50 rounded flex items-center justify-center gap-1 transition"
                    >
        ...
        {activeTab === 'lore' && (
          <div className="space-y-8">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-800/40 p-3 rounded-xl border border-slate-800">
              <div className="flex items-center gap-2 text-xs">
                <BookOpen className="w-3.5 h-3.5 text-purple-400" />
                <span className="text-slate-400">対象プロジェクト:</span>
                <select
                  value={selectedProjectFilter}
                  onChange={(e) => setSelectedProjectFilter(e.target.value)}
                  className="bg-slate-800 border border-slate-700 text-slate-200 rounded-md px-2 py-1 focus:outline-none"
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-1 px-3 py-1.5 bg-purple-600 hover:bg-purple-500 text-white font-semibold text-xs rounded-lg transition"
                >
                  <Plus className="w-3.5 h-3.5" />
                  <span>用語・設定を追加</span>
                </button>
                <button
                  ...
                  className="inline-flex items-center gap-1 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs rounded-lg transition"
                >
                  <Plus className="w-3.5 h-3.5" />
                  <span>章プロットを追加</span>
                </button>
                ...
                {data.lores
                  ...
                  .map((lore) => {
                    ...
                    return (
                      <div
                        key={lore.id}
                        className="bg-slate-800/80 border border-slate-700 rounded-xl p-4 space-y-3 flex flex-col justify-between hover:border-purple-400/50 transition"
                      >
                        <div className="space-y-2">
                          ...
                          {lore.secretNotes && (
                            <div className="p-2.5 rounded-lg bg-slate-900/80 border border-purple-900/40 text-[11px] text-purple-200 space-y-1">
                        ...
                        <div className="pt-2 border-t border-slate-700/60 flex items-center justify-end gap-1">
                          <button
                            onClick={() => {
                              setEditingLore(lore);
                              setLoreModalOpen(true);
              ...
              <div className="space-y-4">
                {data.plots
                  ...
                  .map((plot) => {
                    ...
                    const phaseColors = {
                      introduction: 'bg-emerald-950 text-emerald-300 border-emerald-800',
                      development: 'bg-blue-950 text-blue-300 border-blue-800',
                      twist: 'bg-amber-950 text-amber-300 border-amber-800',
                      climax: 'bg-rose-950 text-rose-300 border-rose-800',
                      resolution: 'bg-purple-950 text-purple-300 border-purple-800'
                    };
                    ...
                    return (
                      <div
                        key={plot.id}
                        className="bg-slate-800/90 border border-slate-700 rounded-xl p-5 space-y-3 hover:border-indigo-400/50 transition"
                      >
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${phaseColors[plot.phase]}`}>
                        ...
                        {plot.keyEvents && plot.keyEvents.length > 0 && (
                          <div className="bg-slate-900/60 p-3 rounded-lg border border-slate-800/80 space-y-1.5">
                            <span className="text-[11px] font-bold text-indigo-300 block">⚡ 主な出来事・シーン:</span>
                        ...
                        {plot.foreshadowingNotes && (
                          <p className="text-[11px] text-amber-300/90 italic flex items-center gap-1.5">
                            <span>💡 伏線・回収メモ:</span>
        ...
        {activeTab === 'export' && (
          <div className="max-w-4xl mx-auto space-y-8">
            <div className="bg-slate-800/80 border border-slate-700 rounded-2xl p-6 space-y-6">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  ブラウザのLocalStorageに保存されている創作プロジェクト・アイデア・設定・タスクデータを自由にバックアップおよびMarkdown形式で書き出せます。
                </p>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                <button
                  ...
                  className="p-4 bg-slate-900 hover:bg-slate-800 border border-slate-700 hover:border-indigo-500 rounded-xl text-left space-y-2 transition group"
                >
                ...
                <label className="p-4 bg-slate-900 hover:bg-slate-800 border border-slate-700 hover:border-indigo-500 rounded-xl text-left space-y-2 transition group cursor-pointer">
              ...
              <div className="pt-4 border-t border-slate-700/60 flex items-center justify-between text-xs">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/40 font-semibold rounded-lg transition"
                >
            ...
            <div className="bg-slate-800/80 border border-slate-700 rounded-2xl p-6 space-y-4">
              ...
              <pre className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-slate-300 text-xs font-mono overflow-x-auto max-h-96 leading-relaxed whitespace-pre-wrap">
      ...
      {detailProjectModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-6 shadow-2xl">
            ...
            <div className="flex items-start justify-between border-b border-slate-800 pb-4">
              <div>
                ...
                {detailProjectModal.subtitle && (
                  <p className="text-xs text-slate-400 mt-0.5">{detailProjectModal.subtitle}</p>
                )}
              </div>
              <button
            ...
            {/* 進捗 & 概要 */}
            <div className="space-y-4">
              <div className="bg-slate-800/60 p-4 rounded-xl space-y-2 border border-slate-700/60">
                <div className="flex justify-between text-xs text-slate-300">
                  <span>制作進捗率</span>
                  <span className="font-bold text-white">{detailProjectModal.progressPercent}%</span>
                </div>
              ...
              <div>
                <h4 className="text-xs font-bold text-slate-400 mb-1">【概要・ストーリー要約】</h4>
              ...
              <div>
                <h4 className="text-xs font-bold text-slate-400 mb-1">【企画コンセプト・世界観テーマ】</h4>
                <p className="text-xs text-slate-200 leading-relaxed whitespace-pre-line bg-slate-950/60 p-3 rounded-xl border border-slate-800">
              ...
              {detailProjectModal.links && detailProjectModal.links.length > 0 && (
                <div>
                  <h4 className="text-xs font-bold text-slate-400 mb-2">【関連リンク・ツール】</h4>
                  <div className="flex flex-wrap gap-2">
                    {detailProjectModal.links.map((lnk, idx) => (
                      <Link
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-900/40 hover:bg-indigo-900/80 text-indigo-300 text-xs rounded-lg border border-indigo-700/50 transition font-medium"
                      >
                        <span>{lnk.title}</span>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {data.ideas.filter(i => i.projectId === detailProjectModal.id).map(idea => (
                    <div key={idea.id} className="p-3 bg-slate-800/80 border border-slate-700 rounded-lg text-xs space-y-1">
                      <div className="flex items-center justify-between font-bold text-white">
                        <span>{idea.title}</span>
                        <span className="text-[10px] text-amber-300">{ideaStatusLabels[idea.status]?.label}</span>
                      </div>
                      <p className="text-slate-300 leading-relaxed">{idea.content}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-5 shadow-2xl">
            ...
            <div className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-300 font-semibold mb-1">プロジェクト名 *</label>
                <input
                  type="text"
                  value={editingProject.title}
                  onChange={(e) => setEditingProject({ ...editingProject, title: e.target.value })}
                  placeholder="例: 星導のエクリプス"
                  className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:border-indigo-500 focus:outline-none"
                />
              </div>

              <div>
                <label className="block text-slate-300 font-semibold mb-1">サブタイトル・キャッチコピー</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">カテゴリ</label>
                  <select
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">ステータス</label>
                  <select
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">作品の概要・ストーリー</label>
                <textarea
                  rows={3}
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">企画コンセプト・テーマ・ターゲット</label>
                <textarea
                  rows={3}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">進捗率 (%) : {editingProject.progressPercent}%</label>
                  <input
                    type="range"
                    min="0"
                    max="100"
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">目標締切日</label>
                  <input
                    type="date"
                    value={editingProject.targetDeadline || ''}
                    onChange={(e) => setEditingProject({ ...editingProject, targetDeadline: e.target.value })}
                    className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2 focus:border-indigo-500 focus:outline-none"
                  />
                </div>
              </div>

              <div>
                <label className="block text-slate-300 font-semibold mb-1">タグ (カンマ区切り)</label>
                <input
                  type="text"
            ...
            <div className="pt-3 border-t border-slate-800 flex justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-semibold rounded-lg shadow-md"
              >
                保存する
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-xl w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setIdeaModalOpen(false)} className="text-slate-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-300 font-semibold mb-1">アイデアタイトル *</label>
                <input
                  type="text"
                  value={editingIdea.title}
                  onChange={(e) => setEditingIdea({ ...editingIdea, title: e.target.value })}
                  placeholder="例: 感情で光が変化する星鉱石のランタン"
                  className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:border-amber-500 focus:outline-none"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:outline-none"
                  >
                    <option value="worldview">🌌 世界観・設定</option>
                    <option value="plot">📜 プロット・展開</option>
                    <option value="character">👤 キャラ・セリフ</option>
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">関連プロジェクト</label>
                  <select
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">アイデア詳細・メモ *</label>
                <textarea
                  rows={4}
                  ...
                  className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:border-amber-500 focus:outline-none leading-relaxed"
                />
              </div>

              <div>
                <label className="block text-slate-300 font-semibold mb-1">発想のきっかけ・インスピレーションメモ</label>
                <input
                  type="text"
            ...
            <div className="pt-3 border-t border-slate-800 flex justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-slate-950 text-xs font-bold rounded-lg shadow-md"
              >
                保存する
              </button>
            </div>
      ...
      {taskModalOpen && editingTask && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-300 font-semibold mb-1">タスク名 *</label>
                <input
                  type="text"
                  value={editingTask.title}
                  onChange={(e) => setEditingTask({ ...editingTask, title: e.target.value })}
                  placeholder="例: 表紙イラストのラフ3案作成"
                  className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:border-sky-500 focus:outline-none"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">対象プロジェクト *</label>
                  <select
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">工程 (レーン)</label>
                  <select
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">詳細説明・作業メモ</label>
                <textarea
                  rows={3}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">優先度</label>
                  <select
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">締切日</label>
                  <input
                    type="date"
            ...
            <div className="pt-3 border-t border-slate-800 flex justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-sky-500 hover:bg-sky-400 disabled:opacity-50 text-slate-950 text-xs font-bold rounded-lg shadow-md"
              >
                保存する
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setLoreModalOpen(false)} className="text-slate-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-300 font-semibold mb-1">用語・設定名 *</label>
                <input
                  type="text"
                  value={editingLore.term}
                  onChange={(e) => setEditingLore({ ...editingLore, term: e.target.value })}
                  placeholder="例: アストラル・コア（星核石）"
                  className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:border-purple-500 focus:outline-none"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">カテゴリ種別</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">設定の説明 *</label>
                <textarea
                  rows={3}
                  value={editingLore.description}
                  onChange={(e) => setEditingLore({ ...editingLore, description: e.target.value })}
                  placeholder="作中での公開設定・基礎知識..."
                  className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:border-purple-500 focus:outline-none leading-relaxed"
                />
              </div>

              <div>
                <label className="block text-purple-300 font-semibold mb-1">🔒 裏設定・伏線メモ（作者用）</label>
                <textarea
                  rows={2}
                  value={editingLore.secretNotes || ''}
                  onChange={(e) => setEditingLore({ ...editingLore, secretNotes: e.target.value })}
                  placeholder="物語後半で明かされる真実や伏線..."
                  className="w-full bg-slate-800 border border-purple-900/50 text-white rounded-lg p-2.5 focus:outline-none"
                />
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-slate-800 flex justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white text-xs font-semibold rounded-lg shadow-md"
              >
                保存する
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setPlotModalOpen(false)} className="text-slate-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-1">
                  <label className="block text-slate-300 font-semibold mb-1">第何章</label>
                  <input
                    type="number"
                ...
                <div className="col-span-2">
                  <label className="block text-slate-300 font-semibold mb-1">章タイトル *</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">起承転結フェーズ</label>
                  <select
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">章のあらすじ・要約 *</label>
                <textarea
                  rows={3}
                  value={editingPlot.summary}
                  onChange={(e) => setEditingPlot({ ...editingPlot, summary: e.target.value })}
                  placeholder="この章で起こることの要約..."
                  className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:border-indigo-500 focus:outline-none leading-relaxed"
                />
              </div>

              <div>
                <label className="block text-slate-300 font-semibold mb-1">主要イベント（改行で箇条書き）</label>
                <textarea
                  rows={3}
              ...
              <div>
                <label className="block text-amber-300 font-semibold mb-1">伏線・回収メモ</label>
                <input
                  type="text"
